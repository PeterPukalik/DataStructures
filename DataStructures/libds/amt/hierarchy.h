...
//				{
//					operation(current);
//					size_t nodeDegree = degree(*current);
//					size_t n = 0;
//					size_t sonsProcessed = 0;
//					while (sonsProcessed < nodeDegree)
//					{
//						BlockType* son = accessSon(*current, n);
//						if (son != nullptr)
//						{
...
//		// po implementacii vymazte vyhodenie vynimky!
//		if (node != nullptr) {
//			processInOrder(accessLeftSon(*node), operation);
//			operation(node);
//			processInOrder(accessRightSon(*node), operation);
//		}
//	}
//
//	template<typename BlockType>
...
//	{
//		if (currentPosition_->currentNode_ || tryFindNextSonInCurrentPosition()) {
//			savePosition(currentPosition_->currentSon_);
//			++(*this);
//		}
//		else {
//			if(currentPosition_->currentNode_) {
//				removePosition();
//
//				if (currentPosition_ != nullptr) {
//					++(*this);
...
namespace ds::amt {
	...
	{
		if (!currentPosition_->currentNode_) {
			if (currentPosition_->currentSonOrder_ != LEFT_SON_INDEX || tryToGoToLeftSonInCurrentPosition()) {
				savePosition(currentPosition_->currentSon_);
				++(*this);
			}
			...
			if (currentPosition_->currentSonOrder_ != RIGHT_SON_INDEX || tryToGoToRightSonInCurrentPosition()) {
				savePosition(currentPosition_->currentSon_);
				++(*this);
			}
